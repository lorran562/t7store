...
import { supabase, Order, fmt } from "@/lib/supabase";
...
export default function AdminPedidos() {
  ...
  if (loading) return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "60vh" }}><div style={{ fontFamily: "var(--font-display)", fontSize: "1.5rem", color: "rgba(245,245,245,0.3)", letterSpacing: "3px" }}>CARREGANDO...</div></div>;

  return (
    <div>
      <div style={{ marginBottom: "28px" }}>
        <h1 style={{ fontFamily: "var(--font-display)", fontSize: "2.2rem", letterSpacing: "2px", color: "#fff" }}>PEDIDOS</h1>
      ...
      <div style={{ display: "flex", gap: "10px", flexWrap: "wrap", marginBottom: "24px" }}>
        {STATUS_OPTIONS.map(s => (
          <button key={s.value} onClick={() => setFilterStatus(s.value === filterStatus ? "all" : s.value)}
            style={{ background: filterStatus === s.value ? `${s.color}22` : "var(--dark2)", border: `1px solid ${filterStatus === s.value ? s.color : "rgba(255,255,255,0.07)"}`, borderRadius: "10px", padding: "10px 16px", cursor: "pointer", color: filterStatus === s.value ? s.color : "rgba(245,245,245,0.5)", fontFamily: "var(--font-body)", fontWeight: 700, fontSize: "0.85rem" }}>
            {s.label} <span style={{ color: s.color, fontFamily: "var(--font-display)" }}>{orders.filter(o => o.status === s.value).length}</span>
          </button>
        ))}
      </div>
      {filtered.length === 0
          ...
          {filtered.map(order => {
            ...
            return (
              <div key={order.id} style={{ background: "var(--dark2)", border: `1px solid ${isOpen ? "rgba(255,255,255,0.12)" : "rgba(255,255,255,0.07)"}`, borderRadius: "14px", overflow: "hidden" }}>
                <div onClick={() => setExpanded(isOpen ? null : order.id)} style={{ display: "flex", alignItems: "center", gap: "16px", padding: "16px 20px", cursor: "pointer" }}>
                  <div style={{ fontFamily: "var(--font-display)", fontSize: "1.1rem", color: "rgba(245,245,245,0.3)", minWidth: "40px" }}>#{order.id}</div>
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontFamily: "var(--font-display)", fontSize: "1.3rem", color: "var(--yellow)" }}>R$ {fmt(Number(order.total))}</div>
                  <select value={order.status} onChange={e => { e.stopPropagation(); updateStatus(order.id, e.target.value); }} onClick={e => e.stopPropagation()}
                    style={{ background: `${st.color}18`, border: `1px solid ${st.color}44`, color: st.color, borderRadius: "20px", padding: "6px 12px", fontSize: "0.78rem", fontFamily: "var(--font-body)", fontWeight: 700, cursor: "pointer", outline: "none" }}>
                  ...
                  <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "16px 20px" }}>
                    {order.items.map((item, i) => (
                      <div key={i} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 14px", background: "rgba(255,255,255,0.03)", borderRadius: "8px", marginBottom: "8px" }}>
                        ...
                        <div style={{ fontFamily: "var(--font-display)", fontSize: "1.1rem", color: "var(--yellow)" }}>R$ {fmt(Number(item.price))}</div>
                      </div>
                    ))}
                  </div>
                )}
